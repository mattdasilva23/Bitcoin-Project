...
def getBTCPrice(c, d):
	return html.unescape(d["bpi"][c]["symbol"]) + str(round(d["bpi"][c]["rate_float"], 1))
...
def printPrices():
	...
	t = pt(["Title", "Value"])				# create table with field values
	t.title = "Bitcoin Table"				# name of the table (will be at the top)
	t.add_row(["Time Requested", myTime])	# add row - time requested
...
# t = pt(["Time Requested", "Bitcoin Price USD", "Bitcoin Price GBP", "Bitcoin Price EUR"])	# headers of the table
# t.add_row([myTime, btcUSD, btcGBP, btcEUR])	# values
